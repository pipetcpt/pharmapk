...
흡수되는 약의 분율(생체이용률, bioavailability: F)은 아래 식 \eqref{eq:eq1}과 같이 나타낸다.
...
몸이 약을 제거하는 능력을 시간당 제거하는 약물의 양(mg/h 와 같은 단위)으로 나타내는 대신, 시간당 그 약이 녹아 있던 혈장이나 혈액을 얼마나 청소해 주었는지(혈액이나 혈장의 부피)로 나타내는 것(L/h 등의 단위)이
청소율(clearance: CL)이다.
즉 용매(혈장)에 녹아 있는 용질(약)을 제거하는 몸의 능력을 용질보다는 용매를 중심으로 표현하는 것이다.
왜 번거롭게 이런 식으로 설명하는가?
그것은 시간당 제거되는 약의 양이란 단위가 약의 혈장 농도가 높을 때는 크다가 농도가 떨어지면 작아져서 일정한 값으로 표현하는 것이 불가능한 반면, 시간당 청소되는 혈액이나 혈장의 부피는 같은 사람에서 약물별로 고정불변의 값을 나타내기 때문이다. 이러한 고정불변의 청소율이 있기 때문에 약물이 1차식에 의해 제거되고 일정한 반감기를 나타내지만,
이를 처음 배우는 사람은 이 개념을 이해하는 것이 어렵다. \index{반감기}
...
비구획분석의 방법론을 이해하고 그로부터 산출 또는 추정되는 약동학 파라미터의 의미를 확인함으로써, 비구획분석 결과의 올바르게 해석하고 활용할 수 있어야 하겠다.
...
{\centering \includegraphics[width=3\linewidth]{media-02/image1} 
...
이 장에서는 PK 자료에 대해 NCA를 수행하는 방법에 대해 자세히 기술하여 능숙하고 정확한 분석을 돕고자 한다. R 소프트웨어를 사용한 프로그래밍이 필요한 부분이 있어 NCA를 수행하기 위해 꼭 알아야 하는 이론은 기술하였으나, 자세한 사항은 별도의 교재를 통해 습득해야 할 것이다. (\protect\hyperlink{ref-ggplot22016}{Wickham 2016})
...
위의 예에서 가장 간단한 형태의 NCA를 실행하였지만, 보다 정밀한 분석은 tblNCA() 함수의 인자를 살펴봄으로서 알 수 있다. \texttt{?tblNCA()} 혹은 \texttt{args(tblNCA)} 명령어를 통해 아래 결과를 얻을 수 있다.
...
\ExtensionTok{Date}\NormalTok{ and Time: 2021{-}06{-}22 15:57:27 Asia/Seoul}
...
\begin{verbatim}
## Error in knitr::include_graphics("media-03/figure-2.png"): Cannot find the file(s): "media-03/figure-2.png"
\end{verbatim}
