...
\#\texttt{\{r\ include\ =\ !is\_html\_output()\}\ \#include\_graphics("images/preface.pdf")\ \#}
...
청소율은 앞서 추정하는 방식을 제시하였으며, 비구획분석에서 \emph{k} (elimination rate constant)에 가장 가까운 지표는 \emph{λ}\textsubscript{z}이다. 따라서, 이 방식에 의하면 이 두 값을 구한 후에 분포용적 값을 추정할 수 있다. 분포용적은 투여된 용량을 어떠한 농도값과 관련지을 것인가에 따라 다양한 방법으로 추정할 수 있는데, 여기에서는 최종제거 구간의 농도들과 연결을 지어 \emph{λ}\textsubscript{z}로부터 이 값을 추정한 것이다. 이렇게 구한 분포용적은 \emph{λ}\textsubscript{z}로부터 추정했다 하여 \emph{V}\textsubscript{z}로 표현한다. 한 가지 추가적으로 언급할 것은 우변의 \emph{CL}과 관련한 것이다. 앞서 대부분의 약동학 연구에서는 \emph{CL}⁄\emph{F}를 구한다 하였으므로, 이를 통해 추정한 분포용적의 값은 엄밀히 \emph{V}\textsubscript{z}/\emph{F}가 되며, 그렇게 표기하는 것이 옳다. 또한, 청소율과 마찬가지로 경구분포용적(oral volume of distribution)이라는 용어를 사용한다.\index{분포용적}
...
반복하지만, 비구획분석에서 \emph{k}에 가장 가까운 지표는 \emph{λ}\textsubscript{z}이다. 따라서, 비구획분석에서는 다음의 식을 통해 반감기 값을 제시한다.\index{반감기}
...
과연 두 약물의 항정상태는 같게 형성될 것인가? 약동학을 어느 정도 공부한\index{항정상태}\index{항정상태}
사람이라면 이 질문에 대한 답이 ``아니오''라는 것은 쉽게 생각해 볼 수 있다.
즉, 같은 \emph{V}\textsubscript{z}가 같은 \emph{C}\textsubscript{ss,avg}를 보장하지 않는다. 그래서,
비구획분석에서는 항정상태분포용적(volume of distribution at\index{항정상태}\index{항정상태}\index{분포용적}
...
\emph{MRT}를 구했다면, 이를 이용하여 \emph{V}\textsubscript{ss}를 구할 수 있다. 다시 항정상태의\index{항정상태}\index{항정상태}
...
약동학(PK) 데이터에 대한 자료해석 시 가장 간단하고도 객관적이며 널리 쓰이는 방법은 비구획 분석 (Non-compartmental analysis, NCA)이고 이는 약물의 임상 개발에 매우 중요하다.
...
실제 채혈시간으로 구성된 자료를 만들기 위해 약동학자는 R 소프트웨어를 주로 사용하게 된다. 방대한 채혈 데이터를 마이크로소프트 엑셀 등을 사용해 다루는 경우 오류의 개입이 될 확률이 커지기 때문이다. 이러한 작업을 도와주는 R 패키지 중 자료를 몇가지 간단한 함수로 빠르게 추출하고 재생성하는 작업에 특화된 dplyr (\protect\hyperlink{ref-R-dplyr}{Wickham et al. 2021}), 자료의 형태를 변형하는 tidyr (\protect\hyperlink{ref-R-tidyr}{Wickham 2021}), 시간 자료를 쉽게 처리할 수 있는 (\protect\hyperlink{ref-R-lubridate}{Spinu, Grolemund, and Wickham 2021}) 등이 있다.
...
이 책에서 주로 다루게 될 \texttt{NonCompart} (\protect\hyperlink{ref-R-NonCompart}{Bae 2020b}), \texttt{ncar} (\protect\hyperlink{ref-R-ncar}{Bae 2019}), \texttt{pkr} (\protect\hyperlink{ref-R-pkr}{Bae and Lee 2018}) 은 NCA를 R을 통해 쉽고 빠르게 행할 수 있는 R 패키지이다.
...
\texttt{ggplot2} 패키지(\protect\hyperlink{ref-R-ggplot2}{Wickham et al. 2020})로 그림을 그려서 대략적인 자료의 모습을 파악할 수 있다. (그림 \ref{fig:ggtheoph}) ggplot2에 대한 설명은 본서의 범위를 벗어나므로 별도의 교재를 통해 학습할 수 있다.
...
\hypertarget{tblnca-uxc804uxccb4-uxb300uxc0c1uxc790-nca}{%
\subsection{tblNCA(): 전체 대상자 NCA}\label{tblnca-uxc804uxccb4-uxb300uxc0c1uxc790-nca}}
...
본 책에 결과를 다 싣기에 지면이 부족하여\footnote{별첨 1에 수록하였음.} dplyr의 \texttt{select()} 함수를 사용하여 핵심적인 일부 파라메터 (C\textsubscript{max}, T\textsubscript{max}, AUC\textsubscript{last}, \(t_{1/2\beta}\))만 표시할 수도 있다.
...
\begin{Highlighting}[]
                       ...
                       \ExtensionTok{Package}\NormalTok{ version 0.4.2 }\ErrorTok{(}\ExtensionTok{2019{-}09{-}27}\NormalTok{ KST}\KeywordTok{)}
                          \ExtensionTok{R}\NormalTok{ version 4.1.0 }\ErrorTok{(}\ExtensionTok{2021{-}05{-}18}\KeywordTok{)}

\ExtensionTok{Date}\NormalTok{ and Time: 2021{-}06{-}17 00:30:29 Asia/Seoul}
...
BE 패키지 중 핵심이 되는 함수는 \texttt{be2x2()} 이다. 현재로서는 2x2 디자인의 간단한 임상시험 디자인만을 지원하고 있으며 이외의 디자인은 sasLM 패키지(\protect\hyperlink{ref-R-sasLM}{Bae 2021})를 사용하여 분석해야 한다. (그림 \ref{fig:twobytwo})
...
\begin{verbatim}
## Error in knitr::include_graphics("media-01/image5.jpg"): Cannot find the file(s): "media-01/image5.jpg"
\end{verbatim}
...
\hypertarget{uxad6cuxd68duxbaa8uxb378uxb85c-uxc5bbuxc740-pk-uxd30cuxb77cuxbbf8uxd130uxc640-ncauxb85c-uxc5bbuxc740-pk-uxd30cuxb77cuxbbf8uxd130uxb4e4uxc758-uxcc28uxc774uxc810}{%
\section{구획모델로 얻은 PK 파라미터와 NCA로 얻은 PK 파라미터들의 차이점*}\label{uxad6cuxd68duxbaa8uxb378uxb85c-uxc5bbuxc740-pk-uxd30cuxb77cuxbbf8uxd130uxc640-ncauxb85c-uxc5bbuxc740-pk-uxd30cuxb77cuxbbf8uxd130uxb4e4uxc758-uxcc28uxc774uxc810}}
...
산출한다. 최종반감기 t\textsubscript{1/2} = 0.693/β로 구한다. 구획분석에서 얻는\index{반감기}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{include\_graphics}\NormalTok{(}\StringTok{"media{-}05/fig5{-}1.png"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
비선형 회귀분석(Nonlinear regression analysis)이 필요한 구획분석을 위해서는 컴퓨터 소프트웨어의 사용이 필수적이다.
...
이에 대한 각종 자료를 이곳에서 다루기는 지면의 제약과 이론의 어려움 때문에 별도의 참고서적을 활용하는 것이 바람직할 것이다.
...
본 서에서는 무료 R 패키지 ``wnl''를 사용한 자료해석과 예제에 대해 다루고자 한다. (\protect\hyperlink{ref-R-wnl}{Bae 2020c})
...
\begin{center}\includegraphics[width=1\linewidth]{pharmapk_files/figure-latex/unnamed-chunk-25-1} \end{center}
...
\NormalTok{\}}
\NormalTok{TIME }\OtherTok{=}\NormalTok{ dPK02[,}\StringTok{"TIME"}\NormalTok{]}
\NormalTok{r2 }\OtherTok{=} \FunctionTok{nlr}\NormalTok{(fPK02b, dPK02, }\AttributeTok{pNames=}\FunctionTok{c}\NormalTok{(}\StringTok{"ka"}\NormalTok{, }\StringTok{"V"}\NormalTok{, }\StringTok{"k"}\NormalTok{, }\StringTok{"tlag"}\NormalTok{), }\AttributeTok{IE=}\FunctionTok{c}\NormalTok{(}\FloatTok{0.1}\NormalTok{, }\DecValTok{30}\NormalTok{, }\FloatTok{0.05}\NormalTok{, }\DecValTok{20}\NormalTok{))}
...
지연 시간이 있는 모형의 적합이 더 좋은 것을 알 수 있다. Cmax 부분을 주의깊게 살펴야 한다.
...
\begin{center}\includegraphics[width=1\linewidth]{pharmapk_files/figure-latex/unnamed-chunk-29-1} \end{center}
...
\FunctionTok{plot}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\AttributeTok{type=}\StringTok{"n"}\NormalTok{, }\AttributeTok{xlim=}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{160}\NormalTok{), }\AttributeTok{ylim=}\FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{, }\DecValTok{1000}\NormalTok{), }\AttributeTok{log=}\StringTok{"y"}\NormalTok{, }\AttributeTok{xlab=}\StringTok{"Time (min)"}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"Concentration (ug/L)"}\NormalTok{)}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\NormalTok{nID) \{}
\NormalTok{  cID }\OtherTok{=}\NormalTok{ IDs[i]}
\NormalTok{  TIME }\OtherTok{=}\NormalTok{ dPK01[dPK01}\SpecialCharTok{$}\NormalTok{ID }\SpecialCharTok{==}\NormalTok{ cID,}\StringTok{"TIME"}\NormalTok{]}
...
\begin{center}\includegraphics[width=1\linewidth]{pharmapk_files/figure-latex/unnamed-chunk-34-1} \end{center}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(wnl)}
\NormalTok{dPK14 }\OtherTok{\textless{}{-}} \FunctionTok{read.csv}\NormalTok{(}\StringTok{"data{-}old/PK14.csv"}\NormalTok{, }\AttributeTok{skip=}\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## Error in colnames(dPK14) = c("TIME", "DV"): object 'dPK14' not found
\end{verbatim}

\begin{verbatim}
## Error in eval(expr, envir, enclos): object 'dPK14' not found
\end{verbatim}
...
아래의 코드는 경구 투여 2구획 분석, 지연시간이 없는 모형이다. Ka, V, k21, alpha, beta 로 농도를 나타낼 수 있다.
...
\NormalTok{                    (k21}\SpecialCharTok{{-}}\NormalTok{Ka)}\SpecialCharTok{/}\NormalTok{(a}\SpecialCharTok{{-}}\NormalTok{Ka)}\SpecialCharTok{/}\NormalTok{(b}\SpecialCharTok{{-}}\NormalTok{Ka)}\SpecialCharTok{*}\FunctionTok{exp}\NormalTok{(}\SpecialCharTok{{-}}\NormalTok{Ka}\SpecialCharTok{*}\NormalTok{T1)) }\CommentTok{\# Erratum in eq 14:1}
...
\begin{verbatim}
## Error in nlr(fPK14a, dPK14, pNames = c("Vc/F", "Ka", "k21", "alpha", "beta"), : object 'dPK14' not found
\end{verbatim}
...
\begin{verbatim}
## Error in nlr(fPK14b, dPK14, pNames = c("Vc/F", "Ka", "k21", "alpha", "beta", : object 'dPK14' not found
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(dPK14[,}\StringTok{"TIME"}\NormalTok{], dPK14[,}\StringTok{"DV"}\NormalTok{], }\AttributeTok{xlim=}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{25}\NormalTok{), }\AttributeTok{ylim=}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{250}\NormalTok{), }\AttributeTok{xlab=}\StringTok{"Time (min)"}\NormalTok{, }\AttributeTok{ylab=}\StringTok{"Concentration (ug/L)"}\NormalTok{, }\AttributeTok{pch=}\DecValTok{16}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## Error in plot(dPK14[, "TIME"], dPK14[, "DV"], xlim = c(0, 25), ylim = c(0, : object 'dPK14' not found
\end{verbatim}
...
## 
## $`Elapsed Time`
## Time difference of 0.02957 secs
## 
## [1] "## ID = 2 ##"
## $Est
##           k      ka        V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.005009 secs
## 
## [1] "## ID = 3 ##"
## $Est
##           k     ka         V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.005803 secs
## 
## [1] "## ID = 4 ##"
## $Est
##            k      ka         V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.004987 secs
## 
## [1] "## ID = 5 ##"
## $Est
##            k      ka        V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.005285 secs
## 
## [1] "## ID = 6 ##"
## $Est
##            k      ka         V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.005613 secs
## 
## [1] "## ID = 7 ##"
## $Est
##            k       ka        V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.006006 secs
## 
## [1] "## ID = 8 ##"
## $Est
##            k      ka         V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.008435 secs
## 
## [1] "## ID = 9 ##"
## $Est
##             k     ka         V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.007078 secs
## 
## [1] "## ID = 10 ##"
## $Est
##            k      ka         V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.007007 secs
## 
## [1] "## ID = 11 ##"
## $Est
##            k     ka         V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.006702 secs
## 
## [1] "## ID = 12 ##"
## $Est
##            k       ka         V AddErrVar AddErrSD
...
## 
## $`Elapsed Time`
## Time difference of 0.00514 secs
\end{verbatim}
...
Bae, Kyun-Seop. 2019. \emph{Ncar: Noncompartmental Analysis for Pharmacokinetic Report}. \url{https://cran.r-project.org/package=ncar}.

\leavevmode\vadjust pre{\hypertarget{ref-R-BE}{}}%
---------. 2020a. \emph{BE: Bioequivalence Study Data Analysis}. \url{https://cran.r-project.org/package=BE}.

\leavevmode\vadjust pre{\hypertarget{ref-R-NonCompart}{}}%
---------. 2020b. \emph{NonCompart: Noncompartmental Analysis for Pharmacokinetic Data}. \url{https://cran.r-project.org/package=NonCompart}.

\leavevmode\vadjust pre{\hypertarget{ref-R-wnl}{}}%
---------. 2020c. \emph{Wnl: Minimization Tool for Pharmacokinetic-Pharmacodynamic Data Analysis}. \url{https://cran.r-project.org/package=wnl}.

\leavevmode\vadjust pre{\hypertarget{ref-R-sasLM}{}}%
---------. 2021. \emph{sasLM: SAS Linear Model}. \url{https://cran.r-project.org/package=sasLM}.
