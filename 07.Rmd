...
|대사|Phase I/II 다른 물질로 변환|Degradation . 펩타이드/아미노산으로 분해|
|배설|담즙 또는 소변 배설. 궁극적인 제거 경로|거의 배설되지 않음. Recycling|
...
### 1. 흡수
항체 의약품은 단백질로 구성되어 있어 위장관의 peptidase와 protease의 높은 활성도에 의해서 위에서 분해되기 쉽다. 또한 수용성의 거대 분자이기 때문에 세포막 투과가 어려워 위장관 점막으로의 흡수 장벽을 통과하기 어렵다. 이러한 이유로 경구 투여에 의해 치료적 목적을 달성하기 어렵고 정맥내 투여, 피하 투여, 근육 투여 등이 선호된다. 피하 투여, 근육 투여가 가장 흔한 투여 경로이지만 생체이용률은 다양하고 낮을 수 있다. 
이러한 이유로 어떤 항체 의약품은 생체 이용률은 비교적 낮으며 이를 개선하기 위한 노력이 지속되고 있다. 
또한 항체 의약품은 피하 투여 후에는 모세혈관 보다는 림프관을 통해 흡수가 일어난다. 
특이한 점은 투여 경로에 따라 제제의 작용이 상이할 수 있는 점이다.
예를 들어, LHRH의 경우 정맥 내 투여의 경우 LH와 FSH의 분지를 촉진하지만 근육 투여 혹은 피하 투여의 경우 LH와 FSH의 분피를 억제하게 된다. 
따라서 항체의약품 개발 시에는 제제의 개발 목적에 맞는 적절한 투여 경로를 고려해야 한다.


### 2. 분포

항체의약품은 크기가 크고 수용성이라 세포막 투과가 어렵기 때문에 체내의 분포는 주로 혈관 내 및 세포외액에 한정되어 있으며 그로 인해 분포용적은 그다지 크지 않은 편이다. 따라서 이론적으로 0.1L/kg 정도의 알부민과 비슷한 정도의 분포용적을 가지며 이는 혈장 부피(0.04L/kg)에서 세포외액 부피(0.23L/kg) 사이 정도의 분포용적를 가진다. 저분자의약품과는 다르게 혈장 단백 결합과는 무관한 특성을 보인다. 
...
항체의약품의 혈관에서부터 조직의 간질액으로의 이동은 주로 대류에 의해 일어난다. 대류 이동은 osmotic pressure에 의해 혈관
또한 IgG의 경우 FcRN과의 binding으로 인한 transcytosis가 bidirectional하게 일어나기 때문에 혈관에서의 extravasation에 영향을 미칠 수 있다. 
...
항체의약품은 혈관으로부터 extravasation한 후 조직의 간질액에서 확산, 대류, 타겟과의 결합 등이 이뤄지면서 분포 정도가 결정된다. 따라서 조직에서의 타겟과의 결합이 기대되는 항체 의약품에서 더 큰 분포용적이 나타날 수 있다. 타겟이 한정되어 있어 포화가 이루어 질 수 있는 경우 비선형적인 분포가 나타날 수 있으며 항정상태의 분포 용적인 항체의 농도에 증가에 따라 감소할 수 있다. 조직 내로 들어간 항체의약품은 림프관을 통해 전신 순환에 도달하게 되는데, 이러한 림프관으로의 이동은 주로 대류에 의해서 일어난다. 대류는 이는 압력, 림프절의 flow rate 그리고 sieving에 의해 결정되며 혈관에서 간질액으로 빠져나올 때와 동일한 방법이나 림프절에서의 pore의 지름이 더 넓기 때문에 혈관에서부터 빠져나올 때보다 더 적은 저항을 갖게 되고 이러한 convection의 차이 때문에 항체의약품은 간질액에서 혈관에서보다 더 낮은 농도를 가지게 된다. 이러한 현상은 endothelial cell의 tight junction이 발달된 경우 더 두드러지게 나타난다. 
따라서 항체의약품의 tissue:plasma concentration ratio (Kp)는 대체로 0.1~0.5정도의 값을 가지며 뇌와 같이 혈관벽의 tight junction이 더 발달된 0.01 이하로 더 낮게 나타날 수 있다. 단, 항체의약품의 분포는 타겟과의 결합이 영향을 미칠 수 있기 때문에 조직에서의 타겟의 분포에 따라서 보다 더 큰 값이 나타날 수도 있다. 


### 4. 제거

항체의약품은 저분자의약품과는 다르게 분자량이 매우 크기 때문에 사구체를 통한 여과가 거의 불가능하다. 따라서 질병이 있는 경우를 제외하고는 소변으로 제거되지 않는다. 항체 의약품의 fragment의 경우 분자량이 비교적 작아 여과가 일어날 수도 있으나 이러한 경우에도 대부분은 재흡수 되거나 근위세뇨관의 세포들에 의해 대사된다. Biliary excretion의 경우 IgG 항체 의약품에서 매우 적은 정도로 나타난다. 따라서 항체 의약품 제거는 대부분 세포내에서의 catabolism에 의해서 나타난다.

세포내에서의 제거는 두가지 형태로 구분할 수 있는대 하나는 비교적 nonspecific한 endocytosis인 pinocytosis를 통해서 lysosomal degradation이 일어나는 경우와 다른 하나는 타겟과의 결합을 통해서 endocytosis가 일어나서 제거되는 경우이다.

#### Pinocytosis에 의한 제거
Pinocytosis는 혈관 벽에 있는 혈관 내 세포에 의해 일어나는 비교적 nonspecific한 fluid-phase endocytosis이다. 혈관벽의 인체 내의 표면 면적이 매우 넓기 때문에 이 과정은 IgG를 빠르게 제거할 수 있다. 따라서 이 과정은 어느 하나의 장기가 아닌 신체 전체에서 나타날 수 있으며 특히 피부, 근육 및 소화 기관 등 모세혈관이 발달되어 혈관 내 세포가 많은 장기에서 두드러지게 나타난다.

이러한 endocytosis 과정은 비교적 nonspecific하기 때문에 모든 단백질 의약품에서 나타날 수 있다. 단, 다른 항체의약품과는 달리 IgG의 경우 uptake이후에 catabolism이 되지 않도록 FcRN recycling이라는 protective mechanism을 가지기 때문에 혈중 농도를 비교적 오래 유지할 수 있다.

#### FcRN Recycling
항체의약품 중 IgG가 pinocytosis과정에서 분해되지 않도록 하는 protective mechanism이다. 
