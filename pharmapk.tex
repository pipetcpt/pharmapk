...
\index{흡수속도상수}\index{생체이용률}\index{분포용적}\index{청소율}
...
NCA 결과는 약물 개발 중 수행 된 많은 임상 약리학 연구(예 : 음식 효과, 약물 상호 작용 및 생물학적 동등성 연구 등)에 대해 중요한 평가기준이 되는 변수를 제공하게 된다.
...
분석 초보자의 경우 유료 강의를 통한 학습을 필요하거나, CDISC\footnote{\url{https://www.cdisc.org}} 표준 용어로 파라메터를 제공하지 않아 그 의미를 따로 파악해야 하는 등의 제한점이 있을 수 있고, 비교적 경험이 쌓인 분석자의 경우 자료 분석 워크플로우를 자동화하는게 어려우며, 윈도우 환경에서만 실행 가능하다는 것도 제한점이 될 수 있다.
...
이들의 기저에 있는 계산 방식은 모두 동일하고 앞서 설명한 상용소프트웨어와 완전히 일치하는 결과를 낼 수 있다. \texttt{NonCompart}의 \texttt{DESCRIPTION} 파일을 보면 다음과 같이 설명하고 있다. 또한 CDISC SDTM 용어를 사용하기 때문에 표준을 준수한다는 장점도 있다.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Theoph\_tblNCA }\OtherTok{\textless{}{-}} \FunctionTok{tblNCA}\NormalTok{(Theoph, }\StringTok{"Subject"}\NormalTok{, }\StringTok{"Time"}\NormalTok{, }\StringTok{"conc"}\NormalTok{, }\AttributeTok{dose=}\DecValTok{320}\NormalTok{, }\AttributeTok{concUnit=}\StringTok{"mg/L"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{Date}\NormalTok{ and Time: 2021{-}06{-}12 01:11:48 Asia/Seoul}
...
ncar::RptCfg의 일부를 첨부한다. (표 \ref{tab:rptcfg}) \texttt{PPTESTCD}는 NonCompart 패키지에서 출력하는 파라메터 이름이며, CDISC SDTM PPTESTCD (Parameter Short Name)\footnote{다음과 같이 CDISC note에 표시되어 있다. `Short name of the pharmacokinetic parameter. It can be used as a column name when converting a dataset from a vertical to a horizontal format. The value in PPTESTCD cannot be longer than 8 characters, nor can it start with a number (e.g., ``1TEST''). PPTESTCD cannot contain characters other than letters, numbers, or underscores. Examples: ``AUCALL,'' ``TMAX,'' ``CMAX.''\,' \url{https://wiki.cdisc.org/pages/viewpage.action?pageId=42309513}}와 같은 값이다. \texttt{WNL} 열은 Certara Phoenix WinNonLin에서 구한 파라메터 이름이다.
...
Lambda z의 계산에 포함되는 값의 시간 하한입니다. LAMZ (터미널 기울기) 계산에 사용 된 가장 빠른 시점이다.

\textbf{Lambda\_z\_upper (LAMZUL)}

Lambda z 계산에 포함되는 값의 시간 상한입니다. LAMZ (터미널 기울기) 계산에 사용 된 마지막 시점이다.
...
\subsection{BE R 패키지를 사용한 생물학적 동등성 판정}\label{be-r-uxd328uxd0a4uxc9c0uxb97c-uxc0acuxc6a9uxd55c-uxc0dduxbb3cuxd559uxc801-uxb3d9uxb4f1uxc131-uxd310uxc815}}

2x2의 생물학적 동등성을 판정하기 위한 가장 간단하고 손쉬운 방법은 Chow와 Liu의 책의 내용(\protect\hyperlink{ref-chow2009design}{Chow and Liu 2008})을 충실히 반영한 BE 패키지(\protect\hyperlink{ref-R-BE}{Bae 2020a})를 사용하는 것이다. 위에서 NCA 관련 패키지를 설치하였듯 BE 패키지를 아래 명령어를 사용하여 설치 및 불러오기 할 수 있다.
...
약동학 파라메터가 입력 자료가 되어 edisonBE 앱을 통해 처리되고 생물학적동등성 판단을 위한 ANOVA 표, 변이 (variability), Least square mean (LSM), geometric mean ratio (GMR)의 90\% 신뢰구간, 샘플 수의 계산이 수행된다. (Figure 2) 본 자료로 계산한 AUClast는 생물학적 동등성 기준을 만족하고 있음을 알 수 있다.
...
\caption{Output format of bioequivalence tests performed by BE R package.}\label{fig:output-format}
\end{figure}



위 과정으로 얻어진 계산값은 가장 정확하게 생물학적 동등성을 평가하고 있어 표준으로 사용되는 SAS 소프트웨어 결과값과 완전히 동일하였다. (Table 3)
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{test2x2}\NormalTok{(NCAResult4BE, }\StringTok{"AUClast"}\NormalTok{)}\SpecialCharTok{$}\StringTok{\textasciigrave{}}\AttributeTok{Sample Size}\StringTok{\textasciigrave{}}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                       True Ratio=1
## 80% Power Sample Size            6
##                       True Ratio=Point Estimate
## 80% Power Sample Size                         7
\end{verbatim}
...
웹브라우저를 통해 간단히 NCA를 할 수 있는 앱이 웹 기반으로 개발되어 R을 설치할 필요없이 사용할 수 있다. \url{https://www.pipetapps.com/project/shiny-ncar/} \texttt{shiny-be}는 NCA 및 생물학적동등성 분석을 웹브라우저에서 할 수 있는 응용 소프트웨어로 \url{https://www.pipetapps.com/project/shiny-be/}에서 서비스 하고 있다.
...
\caption{R 샤이니 앱, \texttt{shiny-be} 접속 화면. \url{http://www.pipetapps.com/shiny-be}}\label{fig:shiny-be}
\end{figure}
...
NCA에서는 AUC로부터 다른 모든 파라미터들을 산출하지만, 그림 \ref{fig:two-comp-ex}과 같이 약물이 정맥주입된 후 중심구획에서 말초구획으로 확산되면서 두 개의 기울기를 나타내는 양상을 NCA에서 얻어진 파라미터들만으로 재현하는 것은 불가능하다.
그러나 이는 그림 \ref{fig:two-comp-ex}의 오른쪽과 같이 두 개의 분포구획(V\textsubscript{c}와 V\textsubscript{p})를 가지는 모델을 상정하고 약물이 중심구획과 말초구획을 k\textsubscript{12}, k\textsubscript{21}의 1차속도상수에 따라 확산되는 것으로 가정하면 잘 설명할 수 있다.
...
경구투여를 하는 경우라면 그림 \ref{fig:fig4-2}와 같은 모델을 흔히 쓰는데 약을 삼켜서 위장관 내로 들어가는 것은 정맥주사(bolus)와 같이 일어나고, 위장관 내강에서 중심구획으로 흡수되는 속도는 위장관 내강에 남아있는 약물의 양에 흡수속도상수인 k\textsubscript{abs}를 곱한 값으로 정의되는 1차흡수를 따른다고 가정하는 방식이 흔히 쓰인다. 이외에 F의 추정이나 흡수의 지체시간(lag time), 1차식 이외의 다른 흡수모델 등을 적용하기 위해 이 모델을 변형할 수 있다.
...
\subsection{최종반감기와 관련한 이슈}\label{uxcd5cuxc885uxbc18uxac10uxae30uxc640-uxad00uxb828uxd55c-uxc774uxc288}}

1장에서 이미 나온 것처럼 최종반감기가 반드시 의미있는 것은 아니다.

그림 \ref{fig:fig4-3}과 같이 붉은 선은 최종반감기가 훨씬 긴(점선) 약임에도 불구하고 항정상태에 도달하는 시간은 검은 선에 비해 훨씬
...
\caption{최종반감기가 더 긴 약이 항정상태에 더 빨리 도달하는 사례}\label{fig:fig4-3}
\end{figure}
