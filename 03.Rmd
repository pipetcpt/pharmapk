...
이 장에서는 PK 자료에 대해 NCA를 수행하는 방법에 대해 자세히 기술하여 능숙하고 정확한 분석을 돕고자 한다. R 소프트웨어를 사용한 프로그래밍이 필요한 부분이 있어 NCA를 수행하기 위해 꼭 알아야 하는 이론은 기술하였으나, 자세한 사항은 별도의 교재를 통해 습득해야 할 것이다. [@ggplot22016]
...
이러한 파일은 `read.csv()` 혹은 `readr`패키지[@R-readr]의 `read_csv()` 함수를 사용해서 자료를 불러 온다.
