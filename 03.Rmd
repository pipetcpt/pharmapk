...
ncar::RptCfg의 일부를 첨부한다. (표 \@ref(tab:rptcfg)) `PPTESTCD`는 NonCompart 패키지에서 출력하는 파라메터 이름이며, CDISC SDTM PPTESTCD (Parameter Short Name)^[다음과 같이 CDISC note에 표시되어 있다. 'Short name of the pharmacokinetic parameter. PPTESTCD cannot contain characters other than letters, numbers, or underscores. Examples: "AUCALL", "TMAX", "CMAX".']와 같은 값이다. `WNL` 열은 Certara Phoenix WinNonLin에서 구한 파라메터 이름이다.
...
(ref:output-format) BE R 패키지에 의해 수행된 생물학적 동등성 통계분석 결과.
...
(ref:shiny-be) NCA용 앱, `pkrshiny` 접속 화면
